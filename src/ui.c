...
int compute_y(int slot);
void ui_provider_add(int provider_id);
void ui_provider_remove(int provider_id);
...
int ui_init(AppContext *ac) {
    ...
    win_providers = newwin(WIN_PROV_H, WIN_WIDTH,
                           WIN_TITLE_H + WIN_GLOBAL_H + 1, WIN_START_X);
    win_trames =
        newwin(WIN_TRAMES_H, WIN_WIDTH,
               1 + WIN_TITLE_H + WIN_GLOBAL_H + WIN_PROV_H + 1, WIN_START_X);
...
void ui_draw(Analyzer *an) {
    ...
    mvwprintw(win_global, 1, 2, "tram_count : %d",
              an->metr.datas.tram_count_total);
    mvwprintw(win_global, 2, 2, "since      : %.1fs",
              elapsed_ms(&an->metr.start) / 1000.0);
    mvwprintw(win_global, 3, 2, "tram/sec   : %d", an->metr.datas.tram_rate);
    ...
    for (uint8_t i = an->providers.reg.instances_actives_count; i-- > 0;) {
        mvwprintw(win_providers, j, 2, "%d", an->providers.entries[i].sa);
        j++;
    }
    ...
    int l = 1;
    for (uint8_t k = an->pgns.reg.instances_actives_count; k-- > 0;) {
        mvwprintw(win_trames, l, 2, "%d", an->pgns.entries[k].frame.pgn);
        mvwprintw(win_trames, l, 11, "%d", an->pgns.entries[k].provider);
        mvwprintw(win_trames, l, 16, "%d",
                  an->pgns.entries[k].metr.datas.tram_rate);
        int m = 20;
        for (uint8_t i = 0; i < an->pgns.entries[k].frame.data_len; i++) {
            mvwprintw(win_trames, l, m, "%02X",
                      an->pgns.entries[k].frame.data[i]);
            m += 3;
        }
        l++;
    }
